...
// yhe data structure for the key space
static struct {
  HMap db;
  // a map of all client collections, keyed by fd 
...
static void *begin_arr(std::string &out) {
  out.push_back(SER_ARR);
  out.append("\0\0\0\0", 4); // filled in end arr
...
static bool try_one_request(Conn *conn) {
  // try to parse a reque fzst from the buffer
  if (conn->rbuf_size < 4) {
...
static void process_timers() {
  // the extra 1000us is for the resolution of poll()
...
static void connection_io(Conn *conn) {
  // waked up by poll, updtae the idle buffer
